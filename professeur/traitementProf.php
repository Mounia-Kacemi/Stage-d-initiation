...
require('../config.php');
	...
	//$tiw = $conn->query("SELECT * FROM professeur INNER JOIN coordonner ON professeur.ID_PROF=cordonner.ID_PROF WHERE Action='Non Supprime' && CIN_PROF='$_POST[cin]' ");
				...
				$req="INSERT INTO professeur(CIN_PROF,NOM_PROF,PRENOM_PROF,TEL_PROF,EMAIL_PROF,SPECIALITE) SELECT '$ci','$nom','$prn',$tel,'$eml','$splt' WHERE NOT EXISTS (SELECT * FROM professeur WHERE CIN_PROF='$ci')";
                    
						mysqli_query($conn,$req);
							$re="SELECT ID_PROF FROM professeur WHERE CIN_PROF='$ci'";
	                        $rsuu= mysqli_query($conn, $re);
	                        $row1=mysqli_fetch_assoc($rsuu);
		                          $prof= $row1["ID_PROF"];              
